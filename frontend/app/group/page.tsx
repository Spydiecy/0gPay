...
function AddrPill({ address, myAddr, client }: { address: string; myAddr: string; client: ReturnType<typeof usePublicClient> }) {
  ...
  useEffect(() => {
    ...
    client.readContract({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'getUser', args: [address as `0x${string}`] })
...
function GroupContributorsInline({ groupId, creator, recipient, amountPerPerson, numParticipants, remarks, myAddr, client }: {
  ...
  const load = useCallback(async () => {
    ...
    setFetching(true);
    try {
      const addrs = await client.readContract({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'getGroupContributors', args: [BigInt(groupId)] }) as string[];
...
function GroupContent() {
  ...
  const handleLookup = useCallback(async () => {
    ...
    try {
      const data = await client?.readContract({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'getGroupPayment', args: [BigInt(contributeId)] }) as GroupData;
  ...
  const handleContribute = useCallback(async (id: string, amtPerPerson: bigint) => {
    ...
    try {
      const hash = await writeContractAsync({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'contributeToGroup', args: [BigInt(id)], value: amtPerPerson });
  ...
  const handleCancel = useCallback(async (id: string) => {
    ...
    try {
      const hash = await writeContractAsync({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'cancelGroupPayment', args: [BigInt(id)] });
      setTxHash(hash);
    } catch (e: unknown) { t(e instanceof Error ? e.message.slice(0, 80) : 'Failed', 'error'); }
    finally { setLoading(false); }
  }, [writeContractAsync]);
  ...
  const handleWithdraw = useCallback(async (id: string) => {
    ...
    try {
      const hash = await writeContractAsync({ address: contractAddress, abi: OG_PAY_ABI, functionName: 'withdrawContribution', args: [BigInt(id)] });
      setTxHash(hash);
    } catch (e: unknown) { t(e instanceof Error ? e.message.slice(0, 80) : 'Failed', 'error'); }
    finally { setLoading(false); }
  }, [writeContractAsync]);
